...
const progressBars = document.getElementsByClassName('progress-bar'),
    ...
    function progressControl(selectedValue, progressPercent){
        for(let i=0; i<buttons.length; i++){
            let eachBtn = buttons[i],
                progress = parseInt(selectedValue.style.width);
    
            eachBtn.addEventListener('click', (event)=>{  
                let controlBtn = parseInt(eachBtn.textContent);
                console.log(progress, controlBtn);
                progress += controlBtn;
...
selector.addEventListener('change', (event)=>{
    ...
    if(selectedValue == 'pb_1'){
        progressControl(pb_1, prog_1);
    } else if(selectedValue == 'pb_2'){
        progressControl(pb_2, prog_2);
    } else if(selectedValue == 'pb_3'){
        progressControl(pb_3, prog_3);
    }
});